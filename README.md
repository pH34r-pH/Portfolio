...
Fleet `main` still has the legacy Azure-bound intake/archive path. The proposed no-Azure replacement in Fleet PR [#350](https://github.com/pH34r-pH/long-haul-fleet/pull/350) is separate and remains draft because `public-artifact-intake` lacks its dedicated App private-key secret; [Fleet #363](https://github.com/pH34r-pH/long-haul-fleet/issues/363) tracks that setup and exact historical no-Azure validation. Do not infer that a candidate was archived from a scheduled poll or successful public build: verify the exact Fleet intake/archive receipt for the selected candidate. The replacement path is not live until its dedicated credential and historical reads pass.
