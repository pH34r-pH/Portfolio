...
## Archived release and recovery

Fleet retains each checked public candidate ZIP and its source receipt as an exact private release before production deployment. The release tag identifies the Portfolio source SHA and public artifact ID; Fleet rechecks the archive SHA-256, extracted bundle digest, source check runs and publication manifest when reading it. The protected `portfolio-production` job records a durable prepared intent before upload, serves the archived bytes through the scoped Portfolio deployment identity, and finalizes only after the apex and `www` HTTPS site, manifest and Lab bytes match. A failed or uncertain upload remains unresolved in the private journal until an operator reconciles its live bytes; a retry cannot silently publish another candidate. Rollback selects the previous successful archived release and restores its original bytes without rebuilding current source. The journal receipt, not a public screenshot artifact or mutable branch name, identifies the currently deployed source and prior rollback input. Fleet #177 and #178 track the private execution and recovery gates.
